...
class ListAuthorsPaginatedInteractorTest {
    private val authorRepository: AuthorRepository = mock()
    private val mapper: ListAuthorsUseCaseMapper = mock()
    private val listAuthorsUseCase = ListAuthorsPaginatedInteractor(authorRepository, mapper)

    @Test
    fun `should return paginated result when authors found`() {
        // Given
        val query = ListAuthorsPaginatedUseCase.Query(page = 1, limit = 2)
        val authors =
            listOf(
    ...
    fun `should return empty result when no authors found`() {
        // Given
        val query = ListAuthorsPaginatedUseCase.Query(page = 1, limit = 10)
    ...
    fun `should handle pagination correctly for second page`() {
        // Given
        val query = ListAuthorsPaginatedUseCase.Query(page = 2, limit = 1)
